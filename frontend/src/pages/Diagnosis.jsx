  ...
  return (
    <div style={{ position: 'relative', width: 160, height: 160, margin: '0 auto' }}>
      ...
      <div style={{
        ...
        <div style={{ fontSize: 40, fontWeight: 700, color, lineHeight: 1 }}>{displayed}</div>
        <div style={{ fontSize: 13, color: 'var(--text-secondary)' }}>/ 100점</div>
      </div>
    </div>
  );
}
...
export default function Diagnosis() {
  ...
  const areaDetails = [
    { key: 'finance_score', label: '재무', icon: '💰', desc: '자산·연금·저축률' },
    { key: 'event_score', label: '생애이벤트', icon: '📅', desc: '이벤트 대비 현금흐름' },
    { key: 'consumption_score', label: '소비패턴', icon: '🛒', desc: '지출 구조·저축 여력' },
    { key: 'health_score', label: '건강', icon: '❤️', desc: '의료비 리스크' },
  ];
    ...
    <div style={{ background: 'var(--bg-page)', minHeight: '100vh' }}>
      ...
      <div style={{ padding: '24px 20px 0' }}>
        ...
        <div className="card-primary" style={{ textAlign: 'center', marginBottom: 16 }}>
          ...
          <p style={{ marginTop: 12, fontSize: 13, opacity: 0.9, lineHeight: 1.6 }}>
            {diagnosis.peer_comparison === '평균보다 낮음' ? '⚠️ ' : diagnosis.peer_comparison === '평균보다 높음' ? '✅ ' : '📊 '}
            {diagnosis.peer_comparison}
          </p>
        </div>
        ...
        <div className="card" style={{ marginBottom: 16 }}>
          <div style={{ display: 'flex', gap: 12, alignItems: 'flex-start' }}>
            <div style={{
              width: 36, height: 36, background: 'var(--primary-light)', borderRadius: '50%',
        ...
        <div className="card" style={{ marginBottom: 16 }}>
          ...
          <ResponsiveContainer width="100%" height={220}>
            <RadarChart data={radarData}>
              ...
              <Radar name="점수" dataKey="score" stroke="var(--primary)" fill="var(--primary)" fillOpacity={0.25} />
            ...
            return (
              <div key={key} style={{ marginBottom: 16 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
                  ...
                  <span style={{ fontWeight: 700, color, fontSize: 18 }}>{score}점</span>
                </div>
        ...
        <div className="card" style={{ marginBottom: 16 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            <div style={{ background: 'var(--bg-section)', borderRadius: 12, padding: '16px', textAlign: 'center' }}>
              <div style={{ fontSize: 12, color: 'var(--text-secondary)', marginBottom: 4 }}>월 부족분</div>
              <div style={{ fontSize: 22, fontWeight: 700, color: 'var(--danger)' }}>
                {diagnosis.monthly_shortfall.toLocaleString()}원
              </div>
            </div>
            <div style={{ background: 'var(--bg-section)', borderRadius: 12, padding: '16px', textAlign: 'center' }}>
              <div style={{ fontSize: 12, color: 'var(--text-secondary)', marginBottom: 4 }}>총 자산 부족분</div>
              <div style={{ fontSize: 22, fontWeight: 700, color: 'var(--danger)' }}>
                {(diagnosis.asset_gap / 10000).toFixed(0)}만원
              </div>
            </div>
          </div>
          {diagnosis.risk_areas.length > 0 && (
            <div style={{ marginTop: 14, padding: '12px 14px', background: '#FFF3F3', borderRadius: 10 }}>
              <p style={{ fontSize: 13, color: 'var(--danger)', fontWeight: 700, marginBottom: 4 }}>⚠️ 취약 영역</p>
